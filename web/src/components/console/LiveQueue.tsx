  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-base truncate" style={{ color: 'var(--navy)' }}>
          {item.clientName}
        </p>
        ...
        {seatError && (
          <p className="text-xs mt-1 font-medium" style={{ color: 'var(--late)' }}>
            {seatError}
          </p>
        )}
      </div>
...
export default function LiveQueue({ queue, onSeat, onNotify }: Props) {
  ...
  return (
    <ul className="flex flex-col gap-3">
      {queue.map((item) => (
        <QueueRow key={item.id} item={item} onSeat={onSeat} onNotify={onNotify} />
      ))}
    </ul>
  );
}
